...
const Registration = () => {
  ...
  const {errorMessageFunction, successMessageFunction} = useContext(AuthContext);
  ...
  function handleRegistration(e: FormEvent<HTMLFormElement>) {
      ...
      if (!email || !password || !username) {
          ...
          if (users.length !== 0) {
              const duplicate = users.find((duplicate: any) => email === duplicate.email || username === duplicate.username);
              if (duplicate) {
