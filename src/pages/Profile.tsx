...
const Profile: React.FC = () => {
  ...
  async function dateChangeSubmit() {
    ...
    if(userData.phone === changedUser.phone && userData.email === changedUser.email) {
      messageService.open({
        type: 'error',
  ...
  return (
    <div className='profileForm'>
      <Title className='profileTitle' level={2}>Change user data</Title>
      <Form
        labelCol={{ span: 6 }}
        wrapperCol={{ span: 14 }}
        layout="horizontal"
      ...
      >
        ...
        <Form.Item className='submitButtonContainer'>
          <Button type='primary' htmlType='submit' disabled={isButtonDisabled()}>Submit Changes</Button>
        </Form.Item>
      </Form>
    </div>
  )
